...
发现新版不等于获得升级授权。正常任务结束时的版本检查只能读取 Release 元数据和候选 tag 精确 SHA 下的 manifest；不能预取或执行远端 upgrader。用户明确提交升级提示中的行动选项，或以“升级brain模板”“升级”等自然语言表达明确意图后，即已授权对应的标准稳定升级：可以从固定 SHA 读取协议文件、分析差异、用非阻塞进度说明告知范围并直接完成写入、复核和检查点，不得再次请求同一升级的确认。用户忽略或未明确同意时，什么也不做。只有前置核查发现破坏性变化、超出标准协议范围、需要未发布开发快照、额外 Git 交付或其他实质性扩权时，才为新增范围另行请求授权。
...
2. 选择并固定权威版本快照。
   - 若授权来自原生选项，先从对应请求核对用户选中的版本。以下稳定版本发现必须与该版本一致；若最高候选已变化，停止写入，说明变化并为新版本重新请求选择，不将旧授权转用于新版本。
...
6. 更新说明文件。
   - `AGENTS.md` 维护初始化状态路由、按层读取、正式接手与其他项目内 skill 路由、长期安全边界和模板协议维护规则；同步「可选提示的交互方式」与原生升级选项，移除要求打字口令的旧升级提示；skill 的详细执行步骤只保留在对应 `SKILL.md` 中。同步时保留下方「项目专属约束」及其项目内容。
...
8. 复核。
   ...
   - 确认正常任务结束时只比较最高有效稳定 Release；main 领先、无更新、网络失败、tag/manifest 不一致或无法判断时不提示；用户未同意时不修改、不阻塞，也不在同轮追问。
   - 确认只有 `brain-initializer`、`brain-handoff` 和 `template-upgrader` 包含 Star 邀请与 PUT 端点；三者都先完成各自核心结果，再按条件静默或显示可选邀请，邀请统一使用原生选项与明确动作措辞，并且只有直接对应当前邀请、账号和仓库的明确提交才构成授权；无合适工具时才使用可忽略的文字入口。标准升级授权不会触发 Star，upgrader 不会在核心升级完成前判定或邀请 Star，也不会为同一标准升级请求第二次确认。
...
10. 核心升级成功后判定并询问可选 Star。
   ...
   - 如用户在本会话中已明确表示不要 Star 且尚未重新提出，跳过检测和邀请，直接报告升级完成。
   ...
   - `can_offer_star=false` 时只交付核心结果。`can_offer_star=true` 且用户尚未单独明确授权 Star 时，按 AGENTS「可选提示的交互方式」，在核心完成结果之后发送独立、可忽略的原生选项请求；同轮若已发送升级提示或其他可选请求，本次不再邀请。
     - 问题文案：“如果 brain-template 让你整理项目、交接工作更省心，愿意点个 Star 支持一下吗？将使用 GitHub 账号 `<账号>` 为 `AaronJiTuo/brain-template` 点 Star。可直接忽略，不影响当前结果或后续使用。”
     - 选项一：`点个 Star 支持`；描述：“使用上面显示的账号为模板仓库点 Star。”
     - 选项二：`暂时不用`；描述：“跳过这次邀请，继续正常使用。”
     - 无合适原生交互时的文字入口：“如果 brain-template 对你有帮助，欢迎说‘帮我点 Star’，我会用 GitHub 账号 `<账号>` 为 `AaronJiTuo/brain-template` 点 Star；可直接忽略，不影响当前结果或后续使用。”
   - 请求只授权 Star，不是确认升级；核心任务已经完成。每轮最多一次可选请求，拒绝后本会话不再主动邀请；不持久化选择。执行前必须把实际提交的动作和邀请中显示的账号、固定仓库对应起来，默认预选、空响应、取消、关闭、超时或忽略都不构成授权。

11. 执行明确授权的 Star。
   - 只有以下任一独立授权成立，且步骤 10 的实时判定为 `can_offer_star=true` 时，才执行：
     - 本 skill 已显示上述 Star 邀请，工具返回用户明确提交的 `点个 Star 支持`，或用户在直接邀请语境中明确同意该 Star 动作；
     - 用户在本次请求中已明确要求升级完成后点 Star。
   - 普通升级确认、其他任务确认或无法可靠对应到当前 Star 邀请的 `确认` 均不构成授权。

   - 执行前重新进行步骤 10 的实时判定；授权来自邀请时，核对当前 GitHub 账号与邀请中显示的账号一致；若账号变化，旧选择不授权新账号操作，停止并说明，只有用户另行明确授权后才继续。用户直接要求 Star 而没有经过邀请时，按其明确指定的账号执行；未指定则使用本次实时判定的活动账号，并在非阻塞进度中说明，不追加同范围确认。已 Star 则不重复 PUT；条件失效时不执行、不重跑核心任务。
   ...
   - 如果行动选择或明确同意来自升级完成后的下一轮回复，不要重跑升级；先确认该回复直接对应上一条 Star 邀请，并从升级 Record 与 CURRENT 核对核心升级已经成功，再重新执行步骤 10 的只读判定，符合条件后执行 PUT。
...
完成一次支持稳定版本检查的协议 bootstrap 后，该项目会在以后每次正常任务结束时静默检查正式 GitHub Releases：没有新版或检查失败时不提示，发现更高的有效稳定版本时按 AGENTS 的原生选项规则询问用户。用户明确同意后即直接完成标准稳定升级，不再为同一升级请求第二次确认；核心升级和检查点成功后，才独立判定是否显示可选 Star 邀请。main 上尚未形成正式 Release 的版本不会触发稳定升级提醒。
