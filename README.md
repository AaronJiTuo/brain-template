...
- 已初始化的 brain 会静默检查正式发布的稳定协议版本；发现新版时优先用当前 agent 的原生选项邀请升级，点选后直接完成标准稳定升级，不重复请求确认。初始化、正式接手或升级成功后，才可能在完成结果中单独显示“点个 Star 支持 / 暂时不用”的可选邀请。两类提示都可直接忽略；不支持合适原生选项的环境保留简短文字入口。详细规则见 [`AGENTS.md`](./AGENTS.md)、[initializer](./.skills/brain-initializer/SKILL.md)、[handoff](./.skills/brain-handoff/SKILL.md) 和 [upgrader](./.skills/template-upgrader/SKILL.md)。
